        ...
        <div className="contenedor">
            ...
            <div className="card">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <input name="cedula" required onChange={handleChange} />

                    <label>Correo Electrónico *</label>
                    <input type="email" name="email" required onChange={handleChange} />

                    <label>Número de Teléfono Celular *</label>
                    <input name="telefono" required onChange={handleChange} />
                    ...
                    <select name="municipio" required onChange={handleChange}>
                    ...
                    <div className="checkbox-group full">
                        {["Comuna 1", "Comuna 2", "Comuna 3", "Comuna 4", "Comuna 5", "Comuna 6", "Comuna 7", "Comuna 8", "Comuna 9", "Sector rural"]
                            .map(z => (
                                <label key={z} className="checkbox">
                                    <input type="checkbox" name="zonaPopayan" value={z} onChange={handleChange} />
                                    <span>{z}</span>
                                </label>
                            ))}
                    </div>
                    ...
                    <input name="barrio" required onChange={handleChange} />

                    <label>¿Qué profesión u oficio tienes?</label>
                    <input name="profesion" onChange={handleChange} />
                    ...
                    <div className="radio-group full">
                        {[
                            ...
                            <label key={op} className="radio">
                                <input type="radio" name="interes" value={op} required onChange={handleChange} />
                                <span>{op}</span>
                            </label>
                        ))}
                    </div>
                    ...
                    <select name="vehiculo" required onChange={handleChange}>
                        <option value="">Seleccione</option>
                        <option>Sí</option>
                        <option>No</option>
                    </select>
                    ...
                    <div className="checkbox-group full">
                        {[
                            ...
                            <label key={r} className="checkbox">
                                <input type="checkbox" name="roles" value={r} onChange={handleChange} />
                                <span>{r}</span>
                            </label>
                        ))}
                    </div>
                    ...
                    <select name="frecuencia" required value={form.frecuencia} onChange={handleChange}>
                    ...
                    <label>
                        Por favor, califique su experiencia o habilidad en el uso de Redes Sociales (Facebook, Instagram, WhatsApp, etc.) para fines políticos o de difusión. *
                    </label>

                    <select name="redes" required value={form.redes} onChange={handleChange}>
                    ...
                    <label className="full">
                        ¿Desea especificar algún comentario adicional o indicar otra forma de apoyo no mencionada?
                    </label>
                    <textarea className="full" name="comentario" onChange={handleChange} />

                    <label className="checkbox full">
                        <input type="checkbox" name="datos" required onChange={handleChange} />
                        <span>
